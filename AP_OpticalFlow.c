...
//int16_t  x,y;
uint32_t last_update =0;       // millis() time of last update
...
void ADNS3080_update_position(float roll, float pitch,float altitude)
{
	...
	vlat += y_cm;
//	if(last_update ==0){
//	        target_x_cm = (int)vlon;
//		target_y_cm = (int)vlat;
...
void position_update(void)
{
     ...
     ADNS3080_update_position(radians(Q_ANGLE.X),radians(Q_ANGLE.Y),current_hight);
}