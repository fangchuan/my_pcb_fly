...
#define log_i       printf
#define log_e       printf
/* cortex-m3 is a 32-bit processor, so abs and labs are equivalent. */
#define labs        abs
...
#define LOAD_CHUNK  (16)
    ...
    for (ii = 0; ii < length; ii += this_write) {
        ...
        if (memcmp(firmware+ii, cur, this_write)){
					        	log_e("Error 3 when load firm\n");
        	log_e("Error loading firmware, ii=%d, length=%d, this_write=%d.\n", ii, length, this_write);
        	log_e("compare result %d\n", memcmp(firmware+ii, cur, this_write));
//         	for(temp=0; temp<this_write; temp++){
//         		log_e("0x%x,  0x%x\n", (firmware+ii)[temp], cur[temp]);
//         	}
            return -2;
				}
    }
