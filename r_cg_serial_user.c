...
static void r_uart0_callback_receiveend(void)
{
    ...
    if(rx_buffer == 0x46)start_receive = 1;
    if(start_receive == 1){
    Rx_Buffer[i++]=rx_buffer;
    if(Rx_Buffer[0] == 0x46 && Rx_Buffer[i-1] == 0x48)
    {
       i =0;
       memset(Commad_Array,0,20);
       memcpy(Commad_Array,Rx_Buffer,Rx_Buffer[2]-0x30+4);
